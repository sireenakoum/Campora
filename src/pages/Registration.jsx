...
label="Curriculum" />
<TabButton active={activeTab === 'calculator'} onClick={() =>
setActiveTab('calculator')} icon={<Calculator size={16} />}
...
const GRADE_SCALE = [
  { min: 93, max: 100, letter: 'A+', points: 4.3 },
  { min: 87, max: 92.9999, letter: 'A', points: 4.0 },
  { min: 83, max: 86.9999, letter: 'A-', points: 3.7 },
  { min: 79, max: 82.9999, letter: 'B+', points: 3.3 },
  { min: 75, max: 78.9999, letter: 'B', points: 3.0 },
  { min: 72, max: 74.9999, letter: 'B-', points: 2.7 },
  { min: 69, max: 71.9999, letter: 'C+', points: 2.3 },
  { min: 66, max: 68.9999, letter: 'C', points: 2.0 },
  { min: 63, max: 65.9999, letter: 'C-', points: 1.7 },
  { min: 61, max: 62.9999, letter: 'D+', points: 1.3 },
  { min: 60, max: 60.9999, letter: 'D', points: 1.0 },
  { min: 0, max: 59.9999, letter: 'F', points: 0.0 }
];
...
const getGrade = value => {
  ...
  return GRADE_SCALE.find(item => numeric >= item.min && numeric <= item.max) || null;
};

const makeGradeEntry = index => ({
  id: `${Date.now()}-grade-${index}-${Math.random()}`,
  name: '',
  numericGrade: ''
});

const makeGpaCourse = index => ({
  id: `${Date.now()}-gpa-${index}-${Math.random()}`,
  name: '',
  credits: '',
  grade: ''
});

const makePreviousCourse = index => ({
  id: `${Date.now()}-previous-${index}-${Math.random()}`,
  semester: '',
  name: '',
  credits: '',
  grade: ''
...
function GradeGpaCalculator() {
  const [mode, setMode] = useState('grade');
  const [gpaMode, setGpaMode] = useState('cumulative');

  const [currentGpa, setCurrentGpa] = useState('');
  const [completedCredits, setCompletedCredits] = useState(() => {
    try {
      return localStorage.getItem('campora-shared-completed-credits') || '';
    } catch {
      return '';
    }
  });
  const [saveCumulativeToSemester, setSaveCumulativeToSemester] = useState(false);
  const [savedSemesterLabel, setSavedSemesterLabel] = useState('');
  ...
  const [previousCourses, setPreviousCourses] = useState(() => {
    try {
      const saved = localStorage.getItem('campora-aub-previous-grades-v2');
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        'campora-aub-previous-grades-v2',
        JSON.stringify(previousCourses)
      );
    } catch {
      // Calculator still works even if storage is unavailable.
    }
  }, [previousCourses]);
  ...
  const addGradeEntry = () => {
    setGradeEntries(current => [...current, makeGradeEntry(current.length + 1)]);
  };

  const removeGradeEntry = id => {
    setGradeEntries(current =>
      current.length > 1
  ...
  const addCourse = () => {
    setCourses(current => [...current, makeGpaCourse(current.length + 1)]);
  };
  ...
  const addPreviousCourse = () => {
    setPreviousCourses(current => [
      ...current,
      makePreviousCourse(current.length + 1)
    ]);
  };

  const updatePreviousCourse = (id, field, value) => {
    setPreviousCourses(current =>
      current.map(course =>
        course.id === id ? { ...course, [field]: value } : course
      )
  ...
  const removePreviousCourse = id => {
    setPreviousCourses(current =>
      current.filter(course => course.id !== id)
    );
  };

  const saveCurrentCoursesToSemesterHistory = () => {
    if (!saveCumulativeToSemester) return;

    const semesterName =
      savedSemesterLabel.trim() || `Semester ${new Date().toLocaleDateString()}`;

    const validCourses = courses.filter(course => {
      const credits = Number(course.credits);
      return course.grade && Number.isFinite(credits) && credits > 0;
    });

    if (!validCourses.length) return;

    const entries = validCourses.map((course, index) => ({
      id: `${Date.now()}-saved-semester-${index}-${Math.random()}`,
    ...
    setPreviousCourses(current => [...current, ...entries]);
  };

  const previousSummary = useMemo(() => {
    const valid = previousCourses.filter(course => {
      const credits = Number(course.credits);
      return course.grade && Number.isFinite(credits) && credits > 0;
    });
    ...
    const qualityPoints = valid.reduce(
      (sum, course) =>
        sum + Number(course.credits) * (LETTER_POINTS[course.grade] ?? 0),
      0
    );

    const semesterMap = {};

    valid.forEach(course => {
      const semesterName = course.semester.trim() || 'Unassigned Semester';
      ...
      semesterMap[semesterName].credits += Number(course.credits);
      semesterMap[semesterName].qualityPoints +=
        Number(course.credits) * (LETTER_POINTS[course.grade] ?? 0);
    });

    const semesters = Object.entries(semesterMap).map(
      ([semester, values]) => ({
        semester,
        credits: values.credits,
        gpa:
          values.credits > 0
            ? values.qualityPoints / values.credits
            : null
      })
    );
  ...
  const gpaSummary = useMemo(() => {
    ...
    const semesterCredits = validCourses.reduce(
      (sum, course) => sum + Number(course.credits),
      0
    );

    const semesterQualityPoints = validCourses.reduce(
      (sum, course) =>
        sum + Number(course.credits) * (LETTER_POINTS[course.grade] ?? 0),
      0
    );
    ...
    const baselineGpa = Number(currentGpa);
    const baselineCredits = Number(completedCredits);

    const hasBaseline =
      Number.isFinite(baselineGpa) &&
      baselineGpa >= 0 &&
      baselineGpa <= 4.3 &&
      Number.isFinite(baselineCredits) &&
      baselineCredits >= 0;

    const baselineQualityPoints =
      hasBaseline ? baselineGpa * baselineCredits : 0;

    const cumulativeCredits =
      (hasBaseline ? baselineCredits : 0) + semesterCredits;

    const cumulativeQualityPoints =
      baselineQualityPoints + semesterQualityPoints;
  ...
  const modeButtonStyle = active => ({
    ...
    fontWeight: 850,
    cursor: 'pointer',
    background: active ? 'var(--campora-navy-solid)' : '#EEF3F8',
    color: active ? '#FFFFFF' : 'var(--campora-navy)',
    boxShadow: active ? '0 7px 18px rgba(10,48,91,0.14)' : 'none',
  ...
  const cardStyle = {
    ...
    borderRadius: '24px',
    padding: '24px',
    boxShadow: '0 10px 30px rgba(15,35,65,0.07)'
  };
  ...
  const sectionToggleStyle = active => ({
    border: active ? '1px solid var(--campora-navy-solid)' : '1px solid #DDE5EE',
    borderRadius: '12px',
    padding: '9px 13px',
    background: active ? 'var(--campora-navy-solid)' : '#FFFFFF',
    color: active ? '#FFFFFF' : 'var(--campora-navy)',
    fontFamily: 'inherit',
    fontSize: '11px',
    fontWeight: 850,
      ...
      <div style={{
        ...
        <div>
          <div style={{
            ...
            color: '#FFFFFF',
            marginBottom: '12px',
            boxShadow: '0 7px 16px rgba(10,48,91,0.14)'
          }}>
            <Calculator size={24} />
          </div>
          ...
          <p style={{
            ...
            Convert grades, save previous grades, and calculate semester or cumulative GPA.
          </p>
        </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={modeButtonStyle(mode === 'grade')}
          >
            Letter Grade
          </button>
          ...
          <button
            type="button"
            onClick={() => setMode('gpa')}
            style={modeButtonStyle(mode === 'gpa')}
          >
            GPA
          </button>
          ...
          <button
            ...
            style={modeButtonStyle(mode === 'previous')}
          >
            Previous Grades
          </button>
        </div>
          ...
          <div style={cardStyle}>
            <div style={{
              ...
              <div>
                ...
                <p style={{
                  ...
                  Course name is optional. Add as many numeric grades as you want.
                </p>
              </div>
            ...
            <div style={{ display: 'grid', gap: '10px' }}>
              {gradeEntries.map((entry, index) => {
                const result = getGrade(entry.numericGrade);
                ...
                return (
                  <div
                    ...
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '42px minmax(180px,1.25fr) minmax(140px,.65fr) minmax(105px,.45fr) minmax(110px,.5fr) 40px',
                      gap: '10px',
                      alignItems: 'end',
                  ...
                  >
                    ...
                    <div>
                      <label style={labelStyle}>
                        COURSE NAME <span style={{ opacity: .65 }}>(OPTIONAL)</span>
                      </label>
                      <input
                        type="text"
                        placeholder="e.g. MECH 201"
                        value={entry.name}
                        onChange={event =>
                          updateGradeEntry(entry.id, 'name', event.target.value)
                        }
                        style={inputStyle}
                      />
                    </div>
                    ...
                    <div>
                      <label style={labelStyle}>NUMERIC GRADE /100</label>
                      <input
                        type="number"
                        min="0"
                        max="100"
                        ...
                        onChange={event =>
                          updateGradeEntry(entry.id, 'numericGrade', event.target.value)
                        }
                        style={inputStyle}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div style={{
                        ...
                        placeItems: 'center',
                        background: '#FFFFFF',
                        border: result ? '1px solid var(--campora-navy-solid)' : '1px solid #DDE6EF',
                        color: result ? 'var(--campora-navy)' : 'var(--campora-muted)',
                        boxShadow: result ? '0 5px 14px rgba(10,48,91,0.08)' : 'none',
                        fontSize: '18px',
                        fontWeight: 950
                      }}>
                    ...
                    <div>
                      <label style={labelStyle}>QUALITY POINTS</label>
                      <div style={{
                        height: '46px',
                        borderRadius: '14px',
                        display: 'grid',
                        ...
                        {result ? result.points.toFixed(1) : '—'}
                      </div>
                    </div>
      ...
      {mode === 'previous' && (
        <div className="stack" style={{ gap: '16px' }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <p style={{
                  ...
                  Optional: save your previous courses by semester and see their GPA automatically.
                </p>
              </div>
              ...
              <div style={{ display: 'grid', gap: '10px' }}>
                {previousCourses.map((course, index) => (
                  <div
                    ...
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '36px minmax(135px,.8fr) minmax(155px,1.2fr) minmax(90px,.45fr) minmax(120px,.6fr) minmax(90px,.4fr) 38px',
                      gap: '9px',
                      alignItems: 'end',
                      border: '1px solid #E5EAF0',
                      borderRadius: '16px',
                  ...
                  >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={event =>
                          updatePreviousCourse(course.id, 'semester', event.target.value)
                        }
                        style={inputStyle}
                      />
                    </div>
                    ...
                    <div>
                      <label style={labelStyle}>
                        COURSE <span style={{ opacity: .65 }}>(OPTIONAL)</span>
                      </label>
                      <input
                        type="text"
                        ...
                        value={course.name}
                        onChange={event =>
                          updatePreviousCourse(course.id, 'name', event.target.value)
                        }
                        style={inputStyle}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={event =>
                          updatePreviousCourse(course.id, 'credits', event.target.value)
                        }
                    ...
                    <div>
                      ...
                      <select
                        value={course.grade}
                        onChange={event =>
                          updatePreviousCourse(course.id, 'grade', event.target.value)
                        }
                        style={inputStyle}
                      >
                        <option value="">Select</option>
                        {Object.keys(LETTER_POINTS).map(letter => (
                          <option key={letter} value={letter}>
                            {letter} · {LETTER_POINTS[letter].toFixed(1)}
                          </option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <div style={{
                        ...
                        background: '#FFFFFF',
                        color: course.grade ? 'var(--campora-navy)' : 'var(--campora-muted)',
                        fontSize: '13px',
                        fontWeight: 900
                      }}>
                        {course.grade
                          ? LETTER_POINTS[course.grade].toFixed(1)
                          : '—'}
                      </div>
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => removePreviousCourse(course.id)}
                      style={{
                        width: '38px',
                        height: '46px',
          ...
          <div style={{
            ...
            <div style={cardStyle}>
              <span style={labelStyle}>PREVIOUS CREDITS SAVED</span>
              <div style={{
                fontSize: '27px',
                fontWeight: 950,
                color: 'var(--campora-text)'
              }}>
                {previousSummary.credits}
              </div>
            </div>
            ...
            <div style={{
              ...
              <span style={labelStyle}>GPA FROM SAVED GRADES</span>
              <div style={{
                fontSize: '36px',
                lineHeight: 1.05,
                fontWeight: 950,
                ...
                {previousSummary.gpa === null
                  ? '—'
                  : previousSummary.gpa.toFixed(2)}
              </div>
              <div style={{
                marginTop: '4px',
                color: 'var(--campora-muted)',
          ...
          {previousSummary.semesters.length > 0 && (
            <div style={cardStyle}>
              <h3 style={{
                margin: '0 0 14px',
                fontSize: '16px',
              ...
              <div style={{
                ...
                {previousSummary.semesters.map(item => (
                  <div
                    key={item.semester}
                    style={{
      ...
      {mode === 'gpa' && (
        <div className="stack" style={{ gap: '16px' }}>
          <div style={cardStyle}>
            <div style={{
              ...
              <div>
                ...
                <p style={{
                  ...
                  Calculate one semester by itself or calculate your new cumulative GPA.
                </p>
              </div>
              ...
              <div style={{
                ...
                <button
                  ...
                  style={sectionToggleStyle(gpaMode === 'cumulative')}
                >
                  Cumulative GPA
                </button>
                ...
                <button
                  ...
                  style={sectionToggleStyle(gpaMode === 'semester')}
                >
                  Semester GPA
                </button>
              </div>
            ...
            {gpaMode === 'cumulative' && (
              <div style={{
                ...
                <div style={{
                  ...
                  <label style={labelStyle}>
                    CURRENT CUMULATIVE GPA / 4.3 <span style={{ color: '#B25568' }}>*</span>
                  </label>
                  ...
                  <div style={{
                    ...
                    Required for an accurate cumulative GPA.
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <label style={labelStyle}>
                    COMPLETED CREDITS <span style={{ color: '#B25568' }}>*</span>
                  </label>
                  ...
                  <div style={{
                    ...
                    This total is shared with Dashboard and Course Management.
                  </div>
                </div>
              </div>
            )}
            ...
            {gpaMode === 'cumulative' && (
              <div style={{
                ...
                <div style={{
                  ...
                  alignItems: 'center',
                  gap: '9px',
                  marginBottom: saveCumulativeToSemester ? '11px' : 0
                }}>
                  <input
                    type="checkbox"
                    checked={saveCumulativeToSemester}
                    onChange={event => setSaveCumulativeToSemester(event.target.checked)}
                    style={{ width: '16px', height: '16px', accentColor: '#0B1A3F' }}
                  />
                  <div>
                    <div style={{
                      fontSize: '11px',
                      ...
                      Save these added courses to Semester GPA history
                    </div>
                    <div style={{
                      marginTop: '2px',
                      fontSize: '10px',
                      ...
                      Optional — leave unchecked if you only want a cumulative GPA calculation.
                    </div>
                  </div>
                </div>

                {saveCumulativeToSemester && (
                  <div style={{
                    display: 'flex',
                    gap: '9px',
                    flexWrap: 'wrap',
                    ...
                    <div style={{ flex: '1 1 220px' }}>
                      ...
                      <input
                        ...
                        value={savedSemesterLabel}
                        onChange={event => setSavedSemesterLabel(event.target.value)}
                        style={inputStyle}
                      />
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={saveCurrentCoursesToSemesterHistory}
                      style={{
                        height: '46px',
                        border: 'none',
                        borderRadius: '12px',
            ...
            {gpaMode === 'cumulative' && previousSummary.gpa !== null && (
              <div style={{
                marginBottom: '16px',
                padding: '12px 14px',
                borderRadius: '14px',
                ...
                Your saved Previous Grades currently calculate to a {previousSummary.gpa.toFixed(2)} GPA across {previousSummary.credits} credits. You can use that as a reference when entering your required current cumulative GPA above.
              </div>
            )}
              ...
              <div>
                <div style={{
                  ...
                  {gpaMode === 'semester'
                    ...
                    : 'Courses to add to your cumulative GPA'}
                </div>
                ...
                <div style={{
                  ...
                  Course name is optional. Credits and letter grade are used in the calculation.
                </div>
              </div>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: '42px minmax(180px,1.45fr) minmax(110px,.5fr) minmax(130px,.62fr) minmax(110px,.5fr) 40px',
              gap: '10px',
              alignItems: 'center',
              padding: '0 13px 7px',
              color: 'var(--campora-muted)',
              ...
              <span style={{ textAlign: 'center' }}>QUALITY POINTS</span>
              <span></span>
            </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{
                      ...
                      display: 'grid',
                      placeItems: 'center',
                      background: course.grade ? '#FFFFFF' : '#F6F7F9',
          ...
          <div style={{
            ...
            <div style={cardStyle}>
              <span style={labelStyle}>
                {gpaMode === 'semester' ? 'SEMESTER CREDITS' : 'NEW COURSE CREDITS'}
              </span>
              <div style={{
                fontSize: '27px',
                fontWeight: 950,
                ...
                {gpaSummary.semesterCredits || '0'}
              </div>
            </div>

            <div style={cardStyle}>
              <span style={labelStyle}>
                {gpaMode === 'semester' ? 'SEMESTER QUALITY POINTS' : 'NEW QUALITY POINTS'}
              </span>
              <div style={{
                fontSize: '27px',
                fontWeight: 950,
                ...
                {gpaSummary.semesterQualityPoints.toFixed(2)}
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                {gpaMode === 'semester'
                  ? (
                    gpaSummary.semesterGpa === null
                      ? '—'
                      : gpaSummary.semesterGpa.toFixed(2)
                  )
                  : (
                    gpaSummary.hasBaseline && gpaSummary.cumulativeGpa !== null
          ...
          {gpaMode === 'cumulative' && !gpaSummary.hasBaseline && (
            <div style={{
              padding: '12px 14px',
              borderRadius: '14px',
              ...
              Enter your current cumulative GPA and completed credits above to calculate your new cumulative GPA.
            </div>
          )}
...
function GradeScaleCard() {
  ...
  return (
    <div style={{
      ...
      borderRadius: '26px',
      padding: '24px',
      boxShadow: '0 12px 32px rgba(15,35,65,0.07)'
    }}>
      <div style={{
        display: 'flex',
        justifyContent: 'space-between',
        ...
        <div style={{
          ...
          borderRadius: '999px',
          background: 'var(--campora-navy-solid)',
          border: '1px solid var(--campora-navy-solid)',
          color: '#FFFFFF',
          fontSize: '11px',
          fontWeight: 850
        }}>
      ...
      <div style={{
        ...
        {rows.map(([numeric, letter, points]) => (
          <div
            key={numeric}
            style={{
              border: '1px solid #E1E8EF',
              ...
              gap: '8px',
              alignItems: 'center',
              boxShadow: '0 3px 10px rgba(15,35,65,0.035)'
            }}
          >
            <div>
              <div style={{
